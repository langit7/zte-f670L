#!/bin/sh
## 07Dec2022
## script login ke router ZTE F670L
## Arif W.A 
...
####<ParaValue>10.144.112.175</ParaValue>
####..
###</Instance>
##</ID_WAN_COMFIG>
#</ajax_response_xml_root>
...
if [ "${wanip%%.*}" = "10" ]; then
    ...
    curl  -s -b cookie.txt "http://192.168.11.1/?_type=menuView&_tag=ethWanConfig&Menu3Location=0&_="$(date +%s)"" \
  -H 'Accept: */*' \
  -H 'Accept-Language: id,en-US;q=0.9,en;q=0.8,gl;q=0.7' \
  -H 'Connection: keep-alive' \
  -H 'DNT: 1' \
  -H 'Referer: http://192.168.11.1/' \
  -H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36' \
  -H 'X-Requested-With: XMLHttpRequest' \
  --compressed \
  --insecure
  
    refresh=$(curl -s -b cookie.txt 'http://192.168.11.1/?_type=menuData&_tag=wan_internet_lua.lua&TypeUplink=2&pageType=0' \
  -H 'Accept: */*' \
  -H 'Accept-Language: id,en-US;q=0.9,en;q=0.8,gl;q=0.7' \
